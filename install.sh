...
sudo docker pull -q mateleo/coolify:latest >/dev/null
echo "Starting Coolify."
cd ~/coolify && sudo docker run -tid --env-file $COOLIFY_CONF_FOUND -v /var/run/docker.sock:/var/run/docker.sock -v coolify-db-sqlite mateleo/coolify:latest /bin/sh -c "env | grep COOLIFY > .env && docker compose up -d --force-recreate" >/dev/null
